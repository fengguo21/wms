<style scoped>
	...
	.content .center .right_form .demo-ruleForm {
		width: 450px;
	}
</style>
<template>
	<div ref="content" class="content" v-on:keydown="sendSubmit()">
		<div class="center">
			...
			<div class="right_form">
				...
				<div class="form">
					<div class="title">药材买卖网仓储管理系统V1.0</div>
...
<script>
	...
	export default {
				...
				_self.$store.dispatch('login', {
					body: {
						biz_param: {
							...
							flag: _self.userInformation.flag
						}
					},
					...
					_self.userInformation=[];
					_self.$router.push('/wms/home/detail');
		...
		created() {
			let _self = this;
			let userInfo = _self.$route.query;
			if(_self.$route.query.str) {
				_self.userInformation.username = userInfo.str.split('-')[0];
				_self.userInformation.password = userInfo.str.split('-')[1];
				_self.userInformation.flag = 'erp';
				_self.login();
