            ...
            <div class="clearfix">
                <el-col :span="8">
                    <el-form-item label="出库类型">
                        <el-select disabled v-model="formData.source" placeholder="请选择">
                ...
                <el-col :span="8">
                    <el-form-item label="仓库名称">
                        <depot :disabled="true" v-model="formData.depotName" v-on:getDepot="getDepot"></depot>
                    </el-form-item>
                </el-col>
                <el-col :span="8">
                ...
                <el-table align="center" max-height="400" :data="formData.newArr" border stripe style="width: 100%; margin: auto" :v-loading.body="loading">
                    <el-table-column label="操作" width="80">
                        <template scope="scope">
                            <el-button @click="deleteResouce({index: scope.$index, id: scope.row.id}, scope.row.isDel)" size="small" icon="delete2" type="text"></el-button>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column prop="numNow" label="预出库量" width="140">
                        <template scope="scope">
                    ...
                    <el-table-column prop="siteName" label="库位点" width="100">
                    </el-table-column>
                </el-table>
            </div>
        </el-form>
...
export default {
    ...
    computed: {
        formData() {
            let obj = this.$store.state.preOutStorage.putTableInfoList;
            //编辑时添加的数据
            let srcList = this.$store.state.preOutStorage.putNewAddResList;
            let target = [];
            let src = obj.stockOutItems;
    ...
    methods: {
        ...
        save(formName) {
            ...
            this.$refs[formName].validate((valid) => {
                if (valid) {
                    //处理资源数组
                    let sendItems = this.filterEditResList();
                    // 处理请求传参
                    let params = {
                    ...
                    let body = {
                            ...
                            biz_method: 'updateStockOutBeforehand',
                            biz_param: params
                        }
                        //加密处理接口
                    ...
                    _self.$store.dispatch('put_updateStockOutBeforehand', obj).then(() => {
                        _self.$emit('newGetHttp');
                        _self.$store.dispatch('put_changDialog', {}).then(() => {
                            _self.getInfoListById(this.formData.id).then(() => {
                                _self.$message({
                                    type: 'success',
                                    message: '修改成功'
                                });
        ...
        deleteResouce(params, origion) {
            ...
            this.$confirm('确定删除该条资源吗？', {
                ...
                if (origion != undefined && origion) {
                    ...
                    let body = {
                        biz_module: 'wmsStockOutService',
                        biz_method: 'deleteStockOutBeforehandItem',
                        biz_param: {
                            itemId: params.id
                        }
                    };
                    ...
                    _self.$store.dispatch('deleteResItems', obj).then(() => {
                        //重新获取数据
                        this.getInfoListById(this.formData.id).then(() => {
                            this.$message({
                                type: 'success',
                                message: '删除成功'
                            });
                    ...
                    _self.$store.dispatch('put_newDelResList', params.index).then(() => {
                        this.$message({
                            type: 'success',
                            message: '删除成功'
                        });
        ...
        filterEditResList() {
            //原来的资源
            let oldRes = this.$store.state.preOutStorage.putTableInfoList.stockOutItems;
            //新增的资源
            let newRes = this.$store.state.preOutStorage.putNewAddResList;
            let newArr = []
        ...
        editNum(index) {
            ...
            this.formData.stockOutItems[index].numNow = 0;
        },
        changeNum(index) {
            let src = this.formData.stockOutItems[index];
            if (src.numNow >= src.numUn) {
                src.numNow = src.numUn
                this.unDisabled = -1;
            }
        },
        //获取指定货主 指定仓库下的资源列表
